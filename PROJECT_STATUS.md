...
| **CI-INT** | CI ran unit only. | **PAID.** `.github/workflows/integration.yml` spins up FalkorDB + Neo4j service containers and runs the integration suite; LLM-dependent tests run when the `MEMRY_LLM_*` repo secrets are set, otherwise skip cleanly. Contributor PRs are now checkable against integration. |
...
- **Not the source-connection / identity-resolution layer.** Agent Memry is the memory +
