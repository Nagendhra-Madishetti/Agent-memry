...
| **EVAL** | verify_fact measured on 4 cases at 0.5 bars. | **CHARACTERIZED (and growing).** Corpus grown to 8 labeled cases; **recall is the tracked headline** (a missed contradiction is the dangerous error for an audit ledger) at >= 0.6, precision >= 0.5. "Characterized" is a moving bar - grow the corpus over time (`tests/integration/test_verify_reliability.py`, `cogniflow.eval`). **Measured floor (2026-07-02, MiniMax-M3, n=8): recall 0.50, precision 1.00 - below the 0.60 bound. The test fails honestly; the bound was NOT lowered to match the result. At n=8 a 0.50-vs-0.60 gap is 1-2 facts (statistically noisy), so the correct fix is to grow the labeled corpus and recalibrate - deferred to a Phase-6 eval-hardening pass, not to move the bar. It is an LLM-quality measurement, isolated from retrieval (unaffected by the Phase-3 embedder/over-fetch changes) and secrets-gated, so it does not run in the default CI job.** |
