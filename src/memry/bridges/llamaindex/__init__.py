"""LlamaIndex bridge : adapters that put the memry substrate behind a
...
LLM), so it is imported only when used - never from ``memry.bridges.__init__``,
which keeps the core import-free. Validity is never re-implemented here; the
postprocessor calls the same ``memry.core.policies`` definition as the substrate.
