...
def create_generator(
    ...
    *,
    ...
    key = (
        api_key
        or os.getenv("MEMRY_GENERATOR_API_KEY")
        or os.getenv("MEMRY_LLM_API_KEY")
        or os.getenv("NVIDIA_API_KEY")
    )
    if not key:
        raise GeneratorError(
            f"generator {name!r} needs an API key; set MEMRY_GENERATOR_API_KEY "
            "(or MEMRY_LLM_API_KEY, or pass api_key)."
        )
...
def create_generator_from_env(max_tokens: int = 2048) -> OpenAICompatibleGenerator:
    """Build a generator from environment (MEMRY_GENERATOR* then MEMRY_LLM_*)."""
    return create_generator(
        os.getenv("MEMRY_GENERATOR", "nvidia"),
        model=os.getenv("MEMRY_GENERATOR_MODEL") or os.getenv("MEMRY_LLM_MODEL"),
        base_url=os.getenv("MEMRY_GENERATOR_BASE_URL") or os.getenv("MEMRY_LLM_BASE_URL"),
        max_tokens=max_tokens,
    )
