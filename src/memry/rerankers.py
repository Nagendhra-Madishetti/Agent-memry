    ...
    def __init__(self, model: str = "BAAI/bge-reranker-v2-m3") -> None: # pragma: no cover
        try:
            ...
            raise RerankerError(
                ...
                "'memry[reranker]'. Or select 'nvidia-rerank' for the hosted option."
            ) from e
        self.model = model
...
def create_reranker(
    ...
    *,
    ...
    if name == "nvidia-rerank":
        key = (
            api_key
            or os.getenv("MEMRY_RERANKER_API_KEY")
            or os.getenv("MEMRY_EMBEDDER_API_KEY")
            or os.getenv("MEMRY_LLM_API_KEY")
            or os.getenv("NVIDIA_API_KEY")
        )
        if not key:
            raise RerankerError(
                "reranker 'nvidia-rerank' needs an API key; set MEMRY_RERANKER_API_KEY "
                "(or MEMRY_EMBEDDER_API_KEY / MEMRY_LLM_API_KEY, or pass api_key)."
            )
