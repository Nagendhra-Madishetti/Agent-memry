...
def complete_from_env(timeout: float = 30.0) -> CompleteFn:
    """Build a bounded OpenAI-compatible ``complete`` from MEMRY_LLM_* env (loads
    ...
    api_key = os.getenv("MEMRY_LLM_API_KEY")
    base_url = (os.getenv("MEMRY_LLM_BASE_URL") or "").rstrip("/")
    model = os.getenv("MEMRY_LLM_MODEL")
