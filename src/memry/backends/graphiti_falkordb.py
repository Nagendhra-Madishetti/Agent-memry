...
it to the memry ``AsyncSubstrate`` contract (write / read / falsify). The LLM
...
backend is actually used (never from ``memry.backends.__init__``), keeping the
core import-free.
"""
...
class GraphitiFalkorDBConfig:
    ...
    # ("bge-m3", "nvidia-e5") are opt-in and require MEMRY_EMBEDDER_API_KEY.
    embedder: str = "hash"
    ...
    @classmethod
    def from_env(cls, group_id: str = "memry") -> GraphitiFalkorDBConfig:
        ...
        return cls(
            ...
            group_id=group_id,
            llm_api_key=os.getenv("MEMRY_LLM_API_KEY"),
            llm_base_url=os.getenv("MEMRY_LLM_BASE_URL"),
            llm_model=os.getenv("MEMRY_LLM_MODEL"),
            embedder=os.getenv("MEMRY_EMBEDDER", "hash"),
            embedder_model=os.getenv("MEMRY_EMBEDDER_MODEL"),
            embedder_api_key=os.getenv("MEMRY_EMBEDDER_API_KEY"),
            embedder_base_url=os.getenv("MEMRY_EMBEDDER_BASE_URL"),
            overfetch_factor=int(os.getenv("MEMRY_OVERFETCH_FACTOR", "10")),
            min_overfetch=int(os.getenv("MEMRY_MIN_OVERFETCH", "50")),
        )
...
class GraphitiFalkorDBBackend:
    """Async :class:`~memry.core.contracts.AsyncSubstrate` over Graphiti+FalkorDB."""
    ...
    async def read(self, query: RetrievalQuery) -> RetrievalResult:
        ...
        if self._last_read_saturated and not self._warned_saturated:
            ...
            _LOG.warning(
                ...
                "ranked below it may be missed. Raise MEMRY_OVERFETCH_FACTOR / "
                "MEMRY_MIN_OVERFETCH or narrow the query.",
                overfetch,
            )
