    ...
    def __init__( # pragma: no cover
        ...
        try:
            ...
            raise EmbedderError(
                ...
                "'memry[embeddings]' (torch + model weights). Or select 'bge-m3' for the "
                "dependency-light hosted option (needs MEMRY_EMBEDDER_API_KEY). Refusing to "
...
NON_SEMANTIC_RETRIEVAL_WARNING = (
    ...
    "MEMRY_EMBEDDER_API_KEY). See the Quickstart. (Hash stays the key-free boot default.)"
)
...
def create_embedder(
    ...
    *,
    ...
    if name in _NVIDIA_MODELS:
        ...
        key = api_key or os.getenv("MEMRY_EMBEDDER_API_KEY") or os.getenv("NVIDIA_API_KEY")
        if not key:
            raise EmbedderError(
                f"embedder {name!r} needs an API key; set MEMRY_EMBEDDER_API_KEY "
