  ...
  return (
    <div className="mx-auto max-w-6xl px-5 py-12">
      ...
      <p className="text-subhead mb-10 max-w-[62ch]">
        Everything to run Agent Memry in your own environment and answer temporally-correct,
      ...
      <div className="grid gap-10 lg:grid-cols-[200px_1fr]">
        ...
        <div className="min-w-0">
          <Doc id="overview" title="Overview">
            <p>
              Agent Memry is a <b>bi-temporal RAG platform</b>: a complete pipeline from any document
...
docker run -d --name memry-db -p 6379:6379 falkordb/falkordb:latest
...
# key-free, needs torch: MEMRY_EMBEDDER=bge-m3-local (pip install -e ".[embeddings]")
# dependency-light, key: MEMRY_EMBEDDER=bge-m3 (needs MEMRY_EMBEDDER_API_KEY)
MEMRY_LLM_API_KEY=... # generation model provider key
MEMRY_LLM_BASE_URL=... # provider base URL
MEMRY_LLM_MODEL=... # model id
MEMRY_EMBEDDER=bge-m3 # semantic retrieval (or 'bge-m3-local' for the key-free path)
MEMRY_EMBEDDER_API_KEY=... # required for bge-m3 (hosted); omit for bge-m3-local

# 4. Run the platform API + the web app
python memry-api/main.py # http://localhost:8000
pnpm -C memry-web dev # http://localhost:3000`}</Code>
          ...
          <Doc id="deploy" title="Deploy (self-host)">
            <p>
              Agent Memry runs entirely in your environment - your infrastructure, your
            ...
            <Code>{`docker run -d --name memry-db -p 6379:6379 \\
  -v memry-data:/data falkordb/falkordb:latest`}</Code>
            <p><b>Platform API.</b> Serve with any ASGI server; restrict CORS to your web origin:</p>
            <Code>{`MEMRY_CORS_ORIGINS=https://app.yourco.com \\
  python -m uvicorn memry-api.main:app --host 0.0.0.0 --port 8000 --workers 4`}</Code>
            <p><b>Web app.</b> Build once and serve the static output; point it at your API:</p>
            <Code>{`NEXT_PUBLIC_API_URL=https://api.yourco.com pnpm -C memry-web build
pnpm -C memry-web start # or deploy the build output to your host`}</Code>
            <p>
            ...
            <p className="mt-3 rounded-lg border border-warn/30 bg-warn/[0.06] p-3 text-sm">
              ...
              <code>MEMRY_API_TOKENS</code>; see <code>SECURITY.md</code>). It is safe in a{" "}
          ...
          <Doc id="using" title="Using the platform">
            <p>
              ...
              were true. Agent Memry parses, chunks, embeds, and writes each fact into the bi-temporal
          ...
          <Doc id="plugins" title="Plugins">
            ...
            <Code>{`MEMRY_EMBEDDER=bge-m3 # or hash (key-free), or a custom endpoint
MEMRY_GENERATOR=... # any OpenAI-compatible provider
MEMRY_RERANKER_API_KEY=... # optional reranker (off by default)
MEMRY_BACKEND_DRIVER=falkordb # or neo4j`}</Code>
