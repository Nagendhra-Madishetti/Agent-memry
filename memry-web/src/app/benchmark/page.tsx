    ...
    <div className="mx-auto max-w-6xl px-5 py-12">
      <Reveal>
        ...
        <p className="text-subhead mb-8 max-w-[68ch]">
          ...
          on these questions, only Agent Memry answers them correctly - the claim is exactly
        ...
        <Reveal delay={0.05}>
          <div className="overflow-hidden rounded-2xl border border-border bg-card elev">
            <table className="w-full text-left text-sm">
              ...
              <tbody>
                {systems.map((s) => {
                  const cf = s.name.startsWith("Agent Memry");
                  return (
          ...
          {systems.map((s) => (
            <details key={s.name} className="group rounded-xl border border-border bg-card elev">
              <summary className="flex cursor-pointer list-none items-center gap-3 px-5 py-4 text-sm">
                <span className={s.name.startsWith("Agent Memry") ? "font-semibold text-brand" : "font-medium"}>
                  {displayName(s.name)}
                </span>
      ...
      <div className="mt-12">
        <Reveal>
          <h2 className="text-section mb-2 flex flex-wrap items-center gap-2">
            How Agent Memry compares to the temporal-RAG field
