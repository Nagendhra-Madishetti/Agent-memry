# Agent Memry Playground API

The live backend for the web app's real-world test: upload real documents → ingest through
the actual Agent Memry pipeline → ask with an as-of time → temporally-correct context + cited
...
python memry-api/main.py # serves http://127.0.0.1:8000
```

Needs a `.env` (repo root) with `MEMRY_LLM_*` and (for real semantic recall)
`MEMRY_EMBEDDER_API_KEY`. Point the web app at it with
...
the web origin (`MEMRY_CORS_ORIGINS`, default `http://localhost:3000`).
