"""Agent Memry Playground API - the live backend for the web app's real-world test.

Upload real documents -> ingest through the ACTUAL Agent Memry pipeline (pypdf/markdown parse ->
...
Run (needs FalkorDB + .env with MEMRY_LLM_* and MEMRY_EMBEDDER_API_KEY):
    pip install -e ".[all,serve]"
    python memry-api/main.py # loopback + open (dev); warns loudly it is open

Baseline security (safe in a TRUSTED environment, NOT enterprise-ready - see SECURITY.md):
    MEMRY_API_TOKENS=tok1,tok2 # require `Authorization: Bearer <tok>` on every route
                                           # except /api/health; each session is scoped to the
                                           # token that created it (no cross-tenant read/reset)
    MEMRY_BIND_HOST=0.0.0.0 # expose off-host (refuses if open + non-loopback)
    MEMRY_RATE_LIMIT_PER_MIN=30 # per-token/IP limit on the LLM/embedder endpoints
    MEMRY_MAX_UPLOAD_BYTES=10485760 # upload cap; MEMRY_PORT sets the port
"""
...
from memry.backends.embedders import available_embedders  # noqa: E402
...
from memry.generators import (  # noqa: E402
    OpenAICompatibleGenerator,
    available_generators,
    create_generator,
    create_generator_from_env,
)
from memry.rerankers import available_rerankers  # noqa: E402
from memry.serving.audit import serialize_belief, serialize_trace  # noqa: E402

DEFAULT_EMBEDDER = "bge-m3" if os.getenv("MEMRY_EMBEDDER_API_KEY") else "hash"
...
_FALKOR_HOST = os.getenv("MEMRY_FALKORDB_HOST", "localhost")
_FALKOR_PORT = int(os.getenv("MEMRY_FALKORDB_PORT", "6379"))

app = FastAPI(title="Agent Memry Playground API", version="0.1.0")
app.add_middleware(
    CORSMiddleware,
...
_LOG = logging.getLogger("memry.api")
_AUTH_TOKENS = {t.strip() for t in os.getenv("MEMRY_API_TOKENS", "").split(",") if t.strip()}
_AUTH_OPEN = not _AUTH_TOKENS # no tokens configured -> unauthenticated (dev) mode
_BIND_HOST = os.getenv("MEMRY_BIND_HOST", "127.0.0.1")
_LOOPBACK = {"127.0.0.1", "localhost", "::1", ""}
_MAX_UPLOAD = int(os.getenv("MEMRY_MAX_UPLOAD_BYTES", str(10 * 1024 * 1024))) # 10 MB
_ALLOWED_SUFFIXES = {".pdf", ".md", ".markdown", ".txt"}
_RATE_LIMIT = int(os.getenv("MEMRY_RATE_LIMIT_PER_MIN", "30"))
_RATE_WINDOW = 60.0
...
if _AUTH_OPEN and _BIND_HOST not in _LOOPBACK and os.getenv("MEMRY_ALLOW_OPEN") != "1":
    raise RuntimeError(
        f"Refusing to start: no MEMRY_API_TOKENS set AND binding non-loopback {_BIND_HOST!r}. "
        "An unauthenticated API on a public interface lets any caller read/wipe any session. Set "
        "MEMRY_API_TOKENS, or bind 127.0.0.1, or (dev only, understood) MEMRY_ALLOW_OPEN=1."
    )
if _AUTH_OPEN:
    _LOG.warning(
        "SECURITY: API running WITHOUT authentication (no MEMRY_API_TOKENS). Anyone who can "
        "reach this port can read/reset any session. OK only on loopback for local dev - set "
        "MEMRY_API_TOKENS before exposing beyond localhost."
    )
...
# MEMRY_SHARED_STATE=1 moves session ownership/config and rate counters into Redis (the
...
_SHARED = os.getenv("MEMRY_SHARED_STATE") == "1"
_RSTATE = None
if _SHARED:
...
async def metrics():
    ...
    body = "\n".join(f"memry_{k} {v}" for k, v in sorted(_METRICS.items()))
...
async def health() -> dict:
    ...
    if not embedder_semantic:
        warnings.append(
            "Retrieval is non-semantic (hash embedder). Set MEMRY_EMBEDDER=bge-m3 with "
            "MEMRY_EMBEDDER_API_KEY (or bge-m3-local) for semantic recall - see the Quickstart."
        )
    return {
        "status": "ok",
        "falkordb": falkordb,
        "llm": bool(os.getenv("MEMRY_LLM_API_KEY")),
...
async def ingest_text(
    ...
    from memry.core.types import Episode, utc_now
...
if __name__ == "__main__":
    ...
    # Loopback by default; set MEMRY_BIND_HOST to expose. The module-level guard above
    # refuses to start unauthenticated on a non-loopback interface unless MEMRY_ALLOW_OPEN=1.
    uvicorn.run(app, host=_BIND_HOST, port=int(os.getenv("MEMRY_PORT", "8000")))
